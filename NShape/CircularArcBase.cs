...
namespace Dataweb.NShape.Advanced {
	...
	public abstract class CircularArcBase : LineShapeBase {
		...
		private float CalcArcCapAngle(int pointIndex, float capSize) {
			...
			foreach (PointF p in Geometry.IntersectCircleArc((float)ctrlPoint.GetPosition().X, (float)ctrlPoint.GetPosition().Y, capSize, StartPoint.X, StartPoint.Y, RadiusPoint.X, RadiusPoint.Y, EndPoint.X, EndPoint.Y, Center.X, Center.Y, Radius)) {
